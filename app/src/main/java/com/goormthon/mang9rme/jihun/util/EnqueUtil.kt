 ...
 * (예시 코드는 AuthService 와 SignInActivity 에 tryPostSignIn() 함수를 참고 하세요)
 */
fun <ResponseType> Call<ResponseType>.enqueueUtil(
    getResultCode: (ResponseType) -> Int, // 기본적인 분기처리를 미리 해놓기 위해 함수 호출시 매번 선언이 필요하다
    onSuccess1000: (ResponseType) -> Unit, // 1000: 작업 성공 코드를 호출 할 때 사용하세요
    /** UI에 error 로직을 구현할 필요가 없을 때를 대비해 기본값으로 null 을 넣어주었다.*/
    onError2000: ((ResponseType) -> Unit)? = null, // 2000 번대 오류를 호출 할 때 사용하세요
    onError3000: ((ResponseType) -> Unit)? = null, // 3000 번대 오류를 호출할 때 사용하세요
    onError4000: ((ResponseType) -> Unit)? = null // 4000 번대 오류를 호출할 때 사용하세요
) {
    ...
    this.enqueue(object : Callback<ResponseType> {
        ...
        override fun onResponse(call: Call<ResponseType>, response: Response<ResponseType>) {
            ...
            when (resultCode) {
                // 작업 성공
                1000 -> onSuccess1000.invoke(response.body() ?: return)

                // 클라이언트 input 에러
                in 2000..3000 -> onError2000?.invoke(response.body() ?: return)

                // db의 의미적 validation 에러
                in 3000..4000 -> onError3000?.invoke(response.body() ?: return)

                // 서버 내부 오류
                else -> onError4000?.invoke(response.body() ?: return)
            }
